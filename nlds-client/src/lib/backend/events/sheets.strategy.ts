...
export class GoogleSheetsStrategy implements SyncStrategy {
  ...
  async execute(event: ExternalEvent): Promise<void> {
    ...
    if (!spreadsheetId || !privateKeyRaw || !clientEmail) {
      console.log(
        `[GOOGLE SHEETS] Missing Credentials/Scope. Skipping synchronisation constraint for ${event.referenceCode}.`,
      );
      return;
    }
    ...
    console.log(
      `[GOOGLE SHEETS] Attempting idempotent push over ReferenceCode: ${event.referenceCode}.`,
    );
