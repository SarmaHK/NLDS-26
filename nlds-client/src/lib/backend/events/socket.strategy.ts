...
export class SocketIoStrategy implements SyncStrategy {
    ...
    async execute(event: ExternalEvent): Promise<void> {
        if (event.type === "REGISTRATION_SUBMITTED") {
            ...
            await realtimePublisher.emit("REGISTRATION_STATUS_CHANGED", statusPayload, `participant:${event.payload.participantId}`);
            await realtimePublisher.emit("REGISTRATION_STATUS_CHANGED", statusPayload, "admin:registrations");
        }
    }
}
