...
export class RegistrationStateMachine {
    static canTransition(current: RegistrationStatus, next: RegistrationStatus): boolean {
        const allowed = ALLOWED_TRANSITIONS[current];
        return allowed?.includes(next) || false;
    }

    static transition(current: RegistrationStatus, next: RegistrationStatus): RegistrationStatus {
