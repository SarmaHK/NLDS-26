...
export default function MissionReadiness() {
  ...
  const currentReadiness = useWatch({
    name: "missionReadiness.readinessLevel",
  });
          ...
          {READINESS_OPTIONS.map((option) => {
            ...
            return (
              <motion.button
                ...
                whileTap={{ scale: 0.99 }}
                className={`dossier-card text-left transition-all w-full p-4 sm:p-5 ${
                  isSelected ? "ring-2 ring-[var(--red)] bg-white/5" : ""
                }`}
                style={{
                  border: isSelected
                    ? "1px solid var(--red)"
              ...
              >
                ...
                <div className="flex items-start gap-4">
                  <div
                    className="flex-shrink-0 flex items-center justify-center w-10 h-10 border border-[var(--border-strong)]"
                    style={{
                      background: isSelected ? "var(--red)" : "transparent",
                    }}
                  >
                    <span
                      className={`font-mono text-lg font-bold ${isSelected ? "text-white" : "text-white/40"}`}
                    >
                      {option.level}
                    </span>
                  </div>
