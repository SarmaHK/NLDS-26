...
export async function GET(request: Request) {
    try {
        ...
        if (!sessionId) return NextResponse.json({ error: "Unauthorized access." }, { status: 401 });

        const activeSession = await prisma.participantSession.findFirst({
            where: { id: sessionId, revokedAt: null, expiresAt: { gt: new Date() } }
        });

        if (!activeSession) return NextResponse.json({ error: "Session expired." }, { status: 401 });

        const statusReport = await prisma.registration.findUnique({
            where: { referenceCode: ref },
            select: { referenceCode: true, status: true, submittedAt: true, participantId: true }
        });

        if (!statusReport || statusReport.participantId !== activeSession.participantId) {
            return NextResponse.json({ error: "Dossier not found or unauthorized access." }, { status: 404 });
        }
        ...
        return NextResponse.json({ error: "Internal Server System Failure." }, { status: 500 });
    }
}
