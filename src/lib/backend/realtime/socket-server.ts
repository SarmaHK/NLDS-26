...
import { ServerToClientEvents, ClientToServerEvents, InterServerEvents, SocketData } from "../../realtime/types";
...
const extractSessionCookie = (cookieString: string | undefined): string | null => {
    if (!cookieString) return null;
    const match = cookieString.match(new RegExp(`(^| )NLDS_SECURE_SESSION=([^;]+)`));
...
io.use(async (socket, next) => {
    try {
        ...
        if (adminSession) {
            ...
            socket.data.permissions = adminSession.admin.permissions.map((p: any) => p.permission);
            return next();
        }
...
app.post("/internal/emit", async (req: Request, res: Response): Promise<any> => {
    const { authSecret, eventName, payload, targetRoom } = req.body;

    if (authSecret !== process.env.SESSION_SECRET) {
        return res.status(403).json({ error: "Unauthorized Realtime Webhook execution." });
    }
...
httpServer.listen(PORT, () => {
    console.log(`[Socket.IO Realtime Engine] Natively spinning abstract boundaries strictly over Port ${PORT}`);
});
